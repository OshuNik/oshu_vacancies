...
const asArr = (v)=>Array.isArray(v) ? v.map(asStr).filter(Boolean) : asStr(v).split(/[;,]\s*/g).map(asStr).filter(Boolean);
...
function normCat(raw) {
  ...
  if (s.includes("ТОЧН")) return "ТОЧНО ТВОЁ";
  if (s.includes("МОЖЕТ")) return "МОЖЕТ БЫТЬ";
  return "НЕ ТВОЁ";
}
...
function extractApplyLinkFromText(rawText) {
  ...
  const parenRe = new RegExp(`${trigger.source}\\s*\\(([^)]+)\\)`, "gi");
  let m;
  while((m = parenRe.exec(text)) !== null){
    const inside = (m[2] || m[1] || "").trim();
    const hit = (inside.match(/(?:https?:\/\/|tg:\/\/)[^\s<>'")]+/i) || [])[0];
    if (hit) return hit;
  }
...
serve(async (req)=>{
  try {
    ...
    if (!supabase || !AI_API_KEY) return json({
    ...
    const row = {
      ...
      text_highlighted,
      category: (()=>{
        const s = asStr(parsed?.category).toUpperCase();
        if (s.includes("ТОЧН")) return "ТОЧНО ТВОЁ";
        if (s.includes("МОЖЕТ")) return "МОЖЕТ БЫТЬ";
        return "НЕ ТВОЁ";
      })(),
